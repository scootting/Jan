<template>
    <div>
        <el-card class="box-card">
            <div slot="header" class="clearfix">
                ...
                <el-button style="text-align: right; float: right" size="small" type="primary" icon="el-icon-plus"
                    @click="addSaleStudentProgram">agregar pago de estudiante</el-button>
            </div>
            <div>
                <el-row>
                    <el-col :span="24">
                        <div class="grid-content bg-purple">
                            <el-form ref="form" :model="dataProgram" label-width="220px" size="small">
                                ...
                                <el-form-item>
                                    <el-button size="small" type="primary" @click.prevent="initCloseFinallyProgram"
                                        plain>Cerrar programa
                                        Academico</el-button>
                                </el-form-item>
                            </el-form>
                        </div>
        ...
        <el-dialog title="detalle del pago" :visible.sync="dialogFormVisible">
            <el-form :model="student" label-width="220px" size="small">
                <el-form-item label="ci" prop="ci_per">
                    <el-input placeholder="ingrese el carnet de identidad" v-model="student.ci_per"
                ...
                <el-form-item label="apellidos y nombres" prop="des_per">
                    <el-input v-model="student.des_per"></el-input>
                </el-form-item>
                <el-form-item label="tipo de pago">
                    <el-select v-model="student.tipo" value-key="descr" size="small"
                        placeholder="seleccione el tipo de documento" @change="OnchangeSale">
                        <el-option v-for="item in dataSale" :key="item.ids" :label="item.detalle" :value="item.ids">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="importe">
                    <el-input v-model="student.importe" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="fecha del registro">
                    <el-date-picker type="date" v-model="student.fecha" placeholder="seleccione una fecha"
                        style="width: 100%" format="yyyy/MM/dd" value-format="yyyy-MM-dd"></el-date-picker>
                </el-form-item>
                <el-form-item label="Numero del comprobante">
                    <el-input v-model="student.comprobante" autocomplete="off"></el-input>
                </el-form-item>
            </el-form>
...
export default {
    data() {
        return {
            dataStudents: [],
            student: { fecha: '', ci_per: '', des_per: '', pago: '', monto: '' },
            dataProgram: {},
            dataSale: [],
    ...
    mounted() {
        this.getProgramById();
        this.getFormSaleProgram(); //tipos de pago
        this.getSaleStudentByProgram();
    },
    methods: {
        //* actualizar un componente al hacer la seleccion nueva *//
        OnchangeSale(idx) {
            let resultado = this.dataSale.find(tipo => tipo.ids == idx);
            this.student.id_metodo = resultado.ids;
            this.student.des_metodo = resultado.detalle;
        },
        addSaleStudentProgram() {
        ...
        async getFormSaleProgram(){
            let app = this;
            try {
                let response = await axios.post("/api/getFormSaleProgram", {
                    id: app.id,
                });
                app.loading = false;
                console.log(response);
                app.dataSale = response.data;
            } catch (error) {
                this.error = error.response.data;
                app.$alert(this.error.message, "Gestor de errores", {
                    dangerouslyUseHTMLString: true,
