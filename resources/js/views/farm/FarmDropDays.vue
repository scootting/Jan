    ...
    <div>
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span>Baja de productos de la granja universitaria</span>
          <el-button style="text-align: right; float: right" size="small" type="primary" icon="el-icon-plus"
            @click="initAddFarmIncomeDay">Nueva baja de productos</el-button>
        </div>
            ...
            <el-table-column align="right" :min-width="350" fixed="right">
              <template slot-scope="scope">
                <el-button :disabled="dataSaleDays[scope.$index].estado == 'V'"
                  ...
                  plain>resumen de baja de productos</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      initIncomeDetailReport(index, row) {
        let transaccion = row;
        axios({
          url: "/api/customerDropDetailReport/",
          params: {
            id: transaccion.id,
